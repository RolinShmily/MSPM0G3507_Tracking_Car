 ...
 *
 * @param hi2c      指向 I2C 外设实例的指针（如 I2C_0_INST）。
 * @param addr      设备的 7 位 I2C 地址。
 * @param regaddr   寄存器地址。
...
///**
//  * @brief OLED写SCL高低电平
//  * @param PinState 要写入SCL的电平值，范围：0/1
//  * @return 无
...
///**
//  * @brief OLED写SDA高低电平
//  * @param PinState 要写入SDA的电平值，范围：0/1
//  * @return 无
...
//	OLED_I2C_SendByte(0X3C);		//发送OLED的I2C从机地址
//	OLED_I2C_SendByte(0x00);		//控制字节，给0x00，表示即将写命令
//	OLED_I2C_SendByte(Command);		//写入指定的命令
//	OLED_I2C_Stop();				//I2C终止
	
	IIC_WriteReg_HW(OLED_INST, 0x3C, 0x00, &Command, 1);
...
//	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		IIC_WriteReg_HW(OLED_INST, 0x3C, 0x40, &Data[i], 1);;	//依次发送Data的每一个数据
	}
//	OLED_I2C_Stop();				//I2C终止
...
/**
  ...
  * @param FontSize 指定字体大小，范围：OLED_8X16（宽8像素，高16像素） 或 OLED_6X8（宽6像素，高8像素）
  * @return 无
  * @note 调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  ...
  * @param IsFilled 指定矩形是否填充，范围：OLED_UNFILLED（不填充） 或 OLED_FILLED（填充）
  * @return 无
  * @note 调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  ...
  * @param IsFilled 指定三角形是否填充，范围：OLED_UNFILLED（不填充） 或 OLED_FILLED（填充）
  * @return 无
  * @note 调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  ...
  * @param IsFilled 指定圆是否填充，范围：OLED_UNFILLED（不填充） 或 OLED_FILLED（填充）
  * @return 无
  * @note 调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  ...
  * @param IsFilled 指定椭圆是否填充，范围：OLED_UNFILLED（不填充） 或 OLED_FILLED（填充）
  * @return 无
  * @note 调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  ...
  * @param StartAngle 指定圆弧的起始角度，范围：-180~180，水平向右为0度，水平向左为180度或-180度，下方为正数，上方为负数，顺时针旋转
  * @param EndAngle 指定圆弧的终止角度，范围：-180~180，水平向右为0度，水平向左为180度或-180度，下方为正数，上方为负数，顺时针旋转
  * @param IsFilled 指定圆弧是否填充，填充后为扇形，范围：OLED_UNFILLED（不填充） 或 OLED_FILLED（填充）
  * @return 无
  * @note 调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
