 ...
 * 反馈: 编码器 10ms 脉冲增量 -> 输出轴 RPM
 *
 * 模式: PID=0 开环 (Sp/+/- 直接调速, 原有行为)
 *       PID=1 闭环 (PID 每 10ms 自动调节左右轮 PWM)
 */

/**
 * @brief 速度闭环模块初始化 (默认关闭, PID 参数需通过串口下发)
 */
void SpeedCtrl_Init(void);

/**
 * @brief 速度闭环周期处理, 需在 SysTick_Handler 中每 1ms 调用一次
 *        内部 10ms 分频执行一次 PID 运算与电机输出更新
 */
void SpeedCtrl_Tick_Handler(void);

/**
 * @brief 使能/关闭速度闭环
 * @param en 1: 使能 (复位 PID 后开始闭环); 0: 关闭 (电机立即停转, 开环 Sp 值清零)
 */
